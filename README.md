...
5. **메뉴바** (`main.cpp: CreateAppMenu`): 실제 Everything.exe에 내장된 문자열을 그대로 뽑아 파일/편집/보기/검색/책갈피/도구/도움말 7개 메뉴를 재현합니다. 파일(열기/경로 열기/경로·전체이름 복사/속성/삭제/새로 고침/닫기)·편집(복사 — 클립보드에 실제 파일 객체로 복사되어 탐색기에 붙여넣기 가능)·보기(창 크기 프리셋, 글꼴 및 색)·검색(대소문자 구분/전체 단어 일치/전체 경로 일치/정규식 사용 — 체크 토글이며 켜져 있으면 타이핑 즉시 모든 검색에 적용)·도구(옵션)·도움말은 실제로 동작합니다. 책갈피와 도구의 나머지 항목은 아직 뒷단이 없어 실제 이름 그대로 보이되 비활성화(회색) 상태입니다. **도구 메뉴에서 ETP/FTP 서버 관련 항목은 사용자 요청에 따라 의도적으로 전부 제외했습니다.**
