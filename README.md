...
- 참고로 새로 만들어진 항목·이름이 바뀐 항목은 실시간 USN 감시로만 반영되고, 아직 크기/날짜는 초기 스캔 때만 채워집니다 (재실행 전까지) — `size:`/`dm:`/`dc:`/`da:` 필터가 그런 새 파일은 "알 수 없음"으로 취급해 걸러냅니다.
- **중복 파일 찾기(`dupe:`)**: 크기/날짜 인덱싱은 끝났으니 다음 단계로 바로 진행 가능.
- **시스템 트레이 + 전역 단축키 + 중복 실행 방지**.
- **설정(제외 폴더) + 인덱스 영속화**: 지금은 실행할 때마다 재인덱싱. 종료 시 인덱스를 디스크에 저장해두면 다음 실행이 즉시 뜸.
- **백그라운드 서비스 + IPC**: 별도 프로세스로 상주시키고 GUI/CLI가 붙었다 떨어졌다 하는 구조 (Everything Service 방식, ETP/FTP/HTTP 서버는 제외하기로 함).
