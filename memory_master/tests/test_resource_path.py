...
def test_resource_path_without_meipass_supports_nested_parts(monkeypatch):
    monkeypatch.delattr(sys, "_MEIPASS", raising=False)
    path = resource_path("icons", "cpu.svg")
    assert path.endswith(os.path.join("resources", "icons", "cpu.svg"))
    assert os.path.exists(path)
...
def test_resource_path_with_meipass_uses_bundle_layout(monkeypatch):
    ...
    path = resource_path("icons", "cpu.svg")
    assert path == os.path.join("fake", "bundle", "dir", "resources", "icons", "cpu.svg")
