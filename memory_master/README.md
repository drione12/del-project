...
- **파일 검색** — 관리자 권한으로 실행 중이면 `src/`의 `EverythingClone.exe`를
  ...
  함께 쓸 수 있음), 이 분류는 `src/`의 EverythingClone과 동일한 기준을 씁니다 - 버튼
  행이 아니라 드롭다운인 것도 EverythingClone 쪽 UI를 그대로 따라간 것으로, 이
  페이지는 EverythingClone 쪽에 새 기능/UI가 생길 때마다 계속 맞춰서 따라가는 것을
  원칙으로 합니다(이번 한 번만의 변경이 아님). 이미지 파일을 선택하면 옆에
  미리보기가 자동으로 크기를 맞춰 표시됩니다. Shift/Ctrl로 여러 항목 다중 선택, Del 키로
  휴지통 이동, 우클릭 메뉴로 열기/
