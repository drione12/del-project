...
list, regardless of any path or name match. Killing any of these can crash
or force-reboot the machine.
...
def is_critical(pid: int, name: Optional[str], self_pid: int) -> bool:
    ...
    unconditionally by the force-delete kill list, so a user can never
